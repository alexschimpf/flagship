...
class TestUpdateContextField(BaseTestCase):
        ...
        with (
            ...
            result = self.run_test_with_user(
                ...
                response_json_modifiers={
                    'context_field_id': context_field.context_field_id,
                    'created_date': context_field.created_date.isoformat()
                }
            )
    ...
    def test_update_context_field__403_project_not_assigned(self) -> None:
        with (
            utils.new_project(project=utils.Project()) as project,
            utils.new_context_field(
                project_id=project.project_id, context_field=utils.ContextField()) as context_field
            ...
            result = self.run_test_with_user(
                runner=self.runner,
                path_to_test_cases=self.path_to_test_cases,
                test_name='test_update_context_field__403_project_not_assigned',
                user=utils.User(),
                url_params={
                    'project_id': project.project_id,
                    'context_field_id': context_field.context_field_id
    ...
    def test_update_context_field__403_read_only_role(self) -> None:
        with (
            utils.new_project(project=utils.Project()) as project,
            utils.new_context_field(
                project_id=project.project_id, context_field=utils.ContextField()) as context_field
            ...
            result = self.run_test_with_user(
                runner=self.runner,
                path_to_test_cases=self.path_to_test_cases,
                test_name='test_update_context_field__403_read_only_role',
    ...
    def test_update_context_field__400_name_taken(self) -> None:
        with (
            utils.new_project(project=utils.Project()) as project,
            utils.new_context_field(
                project_id=project.project_id, context_field=utils.ContextField(name='other')),
            utils.new_context_field(
                project_id=project.project_id, context_field=utils.ContextField()) as context_field
        ):
            result = self.run_test_with_user(
                runner=self.runner,
                path_to_test_cases=self.path_to_test_cases,
                test_name='test_update_context_field__400_name_taken',
                user=utils.User(projects=[project.project_id]),
                url_params={
                    'project_id': project.project_id,
                    'context_field_id': context_field.context_field_id

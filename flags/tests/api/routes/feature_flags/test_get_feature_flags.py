...
class TestGetFeatureFlags(TestCase):
    ...
    def test_get_feature_flags__200(self) -> None:
        signature = self._generate_signature(private_key=self.private_key, user_key='1')
        result = self.runner.run(
            path_to_test_cases=self.path_to_test_cases,
            test_name='test_get_feature_flags__200',
            url_params={'project_id': 1, 'user_key': 1},
            request_header_modifiers={'FLAGSHIP-SIGNATURE': signature},
        )
    ...
    def _generate_signature(private_key: str, user_key: str) -> str:
        return hmac.new(private_key.encode(), user_key.encode(), hashlib.sha256).hexdigest()

    ...
    def test_get_audit_logs__200(self) -> None:
        with (
            utils.new_system_audit_log(
                audit_log=utils.SystemAuditLog(event_type=AuditLogEventType.SET_PASSWORD)
            ) as audit_log_1,
            utils.new_system_audit_log(
                audit_log=utils.SystemAuditLog(event_type=AuditLogEventType.DELETED_PROJECT, details='Project #1')
            ) as audit_log_2,
            utils.new_system_audit_log(
                audit_log=utils.SystemAuditLog(
                    actor='email@email.com', event_type=AuditLogEventType.INVITED_USER, details='Email: other@email.com'
                )
            ) as audit_log_3,
        ):

...
class TestCreateFeatureFlag(BaseTestCase):
    def setUp(self) -> None:
        self.maxDiff = None
    ...
    def test_create_feature_flag__200(self) -> None:
        with (
            ...
            utils.new_context_field(
                project_id=project.project_id,
                context_field=utils.ContextField(
                    field_key='groups', value_type=ContextValueType.ENUM_LIST, enum_def={'admin': 1, 'standard': 2}
                ),
            ),
        ):
            ...
            self.verify_test_result(
                result=result, excluded_response_paths=['feature_flag_id', 'updated_date', 'created_date']
            )

    def test_create_feature_flag__403_project_not_assigned(self) -> None:
        with utils.new_project(project=utils.Project()) as project:
            result = self.run_test_with_user(
                runner=self.runner,
                path_to_test_cases=self.path_to_test_cases,
                test_name='test_create_feature_flag__403_project_not_assigned',
                user=utils.User(),
                url_params={'project_id': project.project_id},
            )
            self.verify_test_result(result=result)

    def test_create_feature_flag__403_read_only_role(self) -> None:
        with utils.new_project(project=utils.Project()) as project:
            result = self.run_test_with_user(
                runner=self.runner,
                path_to_test_cases=self.path_to_test_cases,
                test_name='test_create_feature_flag__403_read_only_role',
                user=utils.User(role=UserRole.READ_ONLY, projects=[project.project_id]),
                url_params={'project_id': project.project_id},
            )
            self.verify_test_result(result=result)
    ...
    def test_create_feature_flag__400_name_taken(self) -> None:
        with (
            utils.new_project(project=utils.Project()) as project,
            utils.new_feature_flag(project_id=project.project_id, feature_flag=utils.FeatureFlag()),
        ):
            result = self.run_test_with_user(
                runner=self.runner,
                path_to_test_cases=self.path_to_test_cases,

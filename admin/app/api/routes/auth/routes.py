...
def login(
    email: str = Form(), password: str = Form(), return_url: str | None = None, authorize: AuthJWT = Depends()
) -> RedirectResponse:
    return LoginController(return_url=return_url, email=email, password=password, authorize=authorize).handle_request()

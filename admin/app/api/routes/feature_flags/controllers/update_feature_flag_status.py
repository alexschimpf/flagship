...
class UpdateFeatureFlagStatusController:
    ...
    def handle_request(self) -> SuccessResponse:
        ...
        conditions = [
            [FeatureFlagCondition(**condition) for condition in and_group] for and_group in row.conditions_list
        ]
        RedisService.add_or_replace_feature_flag(
            project_id=self.project_id, feature_flag_name=row.name, conditions=conditions, is_enabled=row.enabled
        )

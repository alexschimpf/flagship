...
def get_users(page: int = 0, page_size: int = DEFAULT_PAGE_SIZE, me: User = Depends(auth.get_user)) -> Users:
    return GetUsersController(page=page, page_size=page_size, me=me).handle_request()


@router.post('', response_model=User)
def invite_user(request: InviteUser, me: User = Depends(auth.get_user)) -> User:
    return InviteUserController(request=request, me=me).handle_request()
...
def get_user(user_id: int, me: User = Depends(auth.get_user)) -> User:
    return GetUserController(user_id=user_id, me=me).handle_request()


@router.put('/{user_id}', response_model=User)
def update_user(user_id: int, request: UpdateUser, me: User = Depends(auth.get_user)) -> User:
    return UpdateUserController(user_id=user_id, request=request, me=me).handle_request()


@router.delete('/{user_id}', response_model=SuccessResponse)
def delete_user(user_id: int, response: Response, me: User = Depends(auth.get_user)) -> SuccessResponse:
    return DeleteUserController(user_id=user_id, me=me).handle_request(response=response)
...
def set_password(
    ...
    return SetPasswordController(
        email=email, password=password, password_repeat=password_repeat, token=token, authorize=authorize
    ).handle_request()


@router.post('/password/reset', response_model=SuccessResponse)
def reset_password(request: ResetPassword) -> SuccessResponse:
    return ResetPasswordController(request=request).handle_request()

...
class GetContextFieldsController:
    ...
    def handle_request(self) -> ContextFields:
        ...
        context_fields = [ContextField.from_row(row=context_field_row) for context_field_row in context_field_rows]

        return ContextFields(items=context_fields, total=total_count)

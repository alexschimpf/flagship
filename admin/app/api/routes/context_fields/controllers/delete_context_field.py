...
class DeleteContextFieldController:
    ...
    def _validate(self) -> ContextFieldRow:
        if not self.me.role.has_permission(Permission.DELETE_CONTEXT_FIELD) or self.project_id not in self.me.projects:
    ...
    def _delete_context_field(self, context_field_row: ContextFieldRow) -> None:
        with MySQLService.get_session() as session:
            ContextFieldsTable.delete_context_field(
                project_id=self.project_id, context_field_id=self.context_field_id, session=session
            )
            session.add(
                SystemAuditLogRow(
                    actor=self.me.email,
        ...
        RedisService.remove_context_field(project_id=self.project_id, context_field_key=context_field_row.field_key)

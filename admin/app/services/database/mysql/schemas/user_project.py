...
class UsersProjectsTable:
    ...
    def get_all_user_projects(session: Session) -> list[UserProjectRow]:
        return list(session.scalars(select(UserProjectRow)))

    @staticmethod
    def get_user_projects(user_id: int, session: Session) -> list[int]:
        return list(session.scalars(select(UserProjectRow.project_id).where(UserProjectRow.user_id == user_id)))

    @staticmethod
    def update_user_projects(user_id: int, project_ids: list[int], session: Session) -> None:
        session.execute(delete(UserProjectRow).where(UserProjectRow.user_id == user_id))

        session.execute(
            insert(UserProjectRow), [{'user_id': user_id, 'project_id': project_id} for project_id in project_ids]
        )

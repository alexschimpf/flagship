...
class ProjectPrivateKeysTable:
    ...
    def get_project_private_keys(
        project_id: int, page: int, page_size: int, session: Session
    ) -> tuple[list[ProjectPrivateKeyRow], int]:
        rows = list(
            session.scalars(
                select(ProjectPrivateKeyRow)
                .where(ProjectPrivateKeyRow.project_id == project_id)
                .order_by(ProjectPrivateKeyRow.project_id)
        ...
        total_count = cast(
            ...
            session.scalar(
                select(func.count())
                .select_from(ProjectPrivateKeyRow)
                .where(ProjectPrivateKeyRow.project_id == project_id)
            ),
        )
    ...
    def is_project_private_key_name_taken(
        project_id: int, name: str, session: Session, project_private_key_id: int | None = None
    ) -> bool:
        where_conditions = [ProjectPrivateKeyRow.name == name, ProjectPrivateKeyRow.project_id == project_id]
        if project_private_key_id is not None:
            where_conditions.append(ProjectPrivateKeyRow.project_private_key_id != project_private_key_id)

        return bool(
            session.scalar(select(text('1')).select_from(ProjectPrivateKeyRow).where(*where_conditions).limit(1))
        )

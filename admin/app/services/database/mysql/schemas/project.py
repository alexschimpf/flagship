...
class ProjectsTable:
    ...
    def get_project_by_name(name: str, session: Session) -> ProjectRow | None:
        return session.scalar(select(ProjectRow).where(ProjectRow.name == name))

    @staticmethod
    def get_projects(
        project_ids: list[int], page: int, page_size: int, session: Session
    ) -> tuple[list[ProjectRow], int]:
        rows = list(
            session.scalars(
                select(ProjectRow)
                .where(ProjectRow.project_id.in_(project_ids))
    ...
    def update_project(project_id: int, name: str, session: Session) -> None:
        session.execute(update(ProjectRow).where(ProjectRow.project_id == project_id).values({ProjectRow.name: name}))

    @staticmethod
    def are_projects_valid(project_ids: list[int], session: Session) -> bool:
        all_project_ids = set(session.scalars(select(ProjectRow.project_id)) or ())
